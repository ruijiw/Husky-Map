...
public final class RatPoly {
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
	if (!newTerm.getCoeff().equals(RatNum.ZERO)) { // don't add a zero term
	  boolean added = false;
	  for (int i = 0; i < lst.size(); i++) {
		RatTerm term = lst.get(i);
		// find the term with the same exponents
	    if (term.getExpt() == newTerm.getExpt()) {
		  term = term.add(newTerm);
		  // remove the term if it is zero after the addition
		  if (term.equals(RatTerm.ZERO)) { 
		    lst.remove(i); 		
		  } else {
		    lst.set(i, term); 
		  }
		  added = true;
		  break;
		// add the term between two terms
	  ...
	  // if the term is not added yet, add it to the end
	  if (!added) {
		  lst.add(newTerm);
	  }
  	}
  ...
  public RatPoly add(RatPoly p) {
	...
	List<RatTerm> list = new ArrayList<RatTerm>(terms);
  ...
  public RatPoly mul(RatPoly p) {
	  ...
	  List<RatTerm> list = new ArrayList<RatTerm>();
	  for (RatTerm term1: p.terms) {
	     for (RatTerm term2: terms) {
	    	RatTerm mulTerm = term1.mul(term2);
	    	sortedInsert(list,mulTerm);
	     }
	  }
	  return new RatPoly(list);
  }
  ...
  public RatPoly div(RatPoly p) {
	  ...
	  // the degree of remainder is greater than the degree of remainder
	  while(!remainder.terms.isEmpty() && remainder.degree() >= p.degree()){ 
		  RatTerm tr = remainder.getTerm(remainder.degree());			
		  RatTerm tp = p.getTerm(p.degree());					
		  RatTerm quotient = tr.div(tp); 
		  remainder = remainder.sub(p.mul(new RatPoly(quotient)));	
		  sortedInsert(list, quotient);
	  }
	  return new RatPoly(list);
  }
  ...
  public RatPoly differentiate() {
	  ...
	  List<RatTerm> list = new ArrayList<RatTerm>(terms);
	  if (!list.isEmpty()) { 
		  // remove term if it is zero after differentiation
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
	  ...
	  // add the constant if it is not zero
  ...
  public double integrate(double lowerBound, double upperBound) {
	  ...
	  RatPoly antiPoly = this.antiDifferentiate(RatNum.ZERO);
